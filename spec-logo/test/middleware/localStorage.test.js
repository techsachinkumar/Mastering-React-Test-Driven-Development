...
describe('localStorage', () => {
  ...
  describe('save middleware', () => {
    ...
    const state = { script: { present: { name, parsedTokens } } };
  ...
  describe('load', () => {
    ...
    describe('with saved data', () => {
      beforeEach(() => {
        ...
        getItemSpy.mockReturnValueOnce(JSON.stringify([ { a: 123 } ]));
      });
      ...
      it('calls to parsedTokens to retrieve data', () => {
        load();
        expect(parserSpy).toHaveBeenCalledWith([ { a: 123 } ], parser.emptyState);
      });

      it('returns re-parsed draw commands', () => {
        parserSpy.mockReturnValue({ drawCommands: [] });
        expect(load().script.present).toHaveProperty('drawCommands', []);
      });

      it('returns name', () => {
        expect(load().script.present).toHaveProperty('name', 'script name');
      });

      it('returns empty past and future arrays', () => {
        const result = load();
        expect(result.script.past).toEqual([]);
        expect(result.script.future).toEqual([]);
      });
    });
